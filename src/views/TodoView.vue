      ...
      >
        ...
        >
          ...
          <v-list-item-content>
            ...
            size="15px"
            >
                <Avatar :username="todo.victimid" :size="15" />
              </v-avatar>
              {{ todo.victimid }}
            </v-list-item-text>
          </v-list-item-content>
...
<!-- No Todos if todo list is empty -->
    ...
    <!-- No Todos if todo list is empty -->
    <div v-else>
      <div id="no-todo-bg">NO TODOS</div>
    </div>
    <!-- Delete Modal -->
    <DeleteModal
...
export default {
  ...
  methods: {
    async addTodo() {
      ...
      this.newTodoMessage = await this.$store.dispatch(
        "getTodos",
    ...
    async deleteTodo(id) {
      ...
      this.$store.state.todos = this.$store.state.filter(
        (todo) => todo.id !== id
      );
      this.todos = this.$store.state.todos;
    ...
    deleteFromModal() {
      this.$store
        .dispatch("deleteTodo", this.currentId)
        .then(() => {
