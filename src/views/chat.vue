        ...
        <section clas="chat-box">
            ...
            <v-card
                 ...
                 max-width="200">
                     ...
                     <v-card-subtitle class="username">{{message.username}}</v-card-subtitle>
                     <v-chip class="mb-2 mr-4" 
                     :color="(message.username == username ? 
            'primary' 
            : 
            'light-green accent-1')">{{message.content}}</v-chip>
...
export default {
    ...
    methods: {
        ...
        sendMessage(){
               const messagesRef = firebase.database().ref(this.$store.state.testUser.house_key)
